...
parser.add_argument('--warmup', type=int, default=50, help='Location of dataset')                    
        ...
        for loader in npyList1:
            ...
            for i, img in enumerate(train_loader):
                ...
                if (epoch >= args.warmup and epoch % 10 == 0):
                    #  print('Slam')
                    gt        = (from_polar(img).cuda()).reshape(-1,3,args.beam * int(512/args.dim) ).permute(0,2,1).cuda()
                    recon_new = from_polar(recon[:,:,:args.beam,:]).reshape(-1,3,  args.beam * int(512/args.dim) ).permute(0,2,1).cuda()
                    # print(gt.shape, recon_new.shape)
                    # exit(0)

                    groundtruth_list=  [gt[i] for i in range(img.shape[0])]
                    recon_list      =  [recon_new[i] for i in range(recon.shape[0])] 
