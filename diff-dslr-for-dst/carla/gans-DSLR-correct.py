...
parser.add_argument('--beam', type=int, default=16,help='Location of dataset')                                          
...
def loadNP(index):
    ...
    for i in trange(index):
        static.append( np.load( DATA  + "lidar/s{}.npy".format(i) )[:,:,::int(64/args.beam),::args.dim].astype('float32'))
        dynamic.append( np.load( DATA + "lidar/d{}.npy".format(i) )[:,:,::int(64/args.beam),::args.dim].astype('float32'))
    ...
    for i in npyList1:
        ...
        while iters < len(i):
            ...
            while j < args.dis_iters and iters < len(i):
                ...
                recon0, kl_cost0, z_real0 = gen(process_input(inputs[1][:,:,:args.dim,:].cuda()))
                recon1, kl_cost1, z_real1 = gen(process_input(inputs[2][:,:,:args.dim,:].cuda()))                
            ...
            # noise = torch.cuda.FloatTensor(args.batch_size, 100).normal_()
            recon2, kl_cost2, z_fake2 = gen(process_input(inputs[3][:,:,:args.dim,:].cuda()))
            recon1, kl_cost1, z_real1 = gen(process_input(inputs[2][:,:,:args.dim,:].cuda()))
            recon0, kl_cost0, z_real0 = gen(process_input(inputs[1][:,:,:args.dim,:].cuda()))
            ...
            recloss = loss_fn(recon2[:,:,:args.dim,:], inputs[1][:,:,:args.dim,:].cuda()).mean(dim=0)
            gen_loss += recloss

            if (epoch >= args.warmup and epoch % 5 == 0):
                #  print('Slam')
                gt        = from_polar(inputs[1].cuda()).reshape(-1,3,args.beam * int(1024/args.dim)).permute(0,2,1)
                recon_new = from_polar(recon2[:,:,:args.dim,:]).reshape(-1,3,args.beam * int(1024/args.dim)).permute(0,2,1)
